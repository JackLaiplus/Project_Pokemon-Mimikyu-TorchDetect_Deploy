...
專案簡介，`Project_Pokemon-Mimikyu-TorchDetect` 是一個基於 **TensorFlow/Keras** 的圖像分類專案，專門用來辨識寶可夢 **Mimikyu** 與 **Pikachu**。Mimikyu（日語：ミミッキュ）是一種會偽裝成 Pikachu 的寶可夢，目的在於獲得更多朋友。由於外觀相似，對模型來說辨識具有挑戰性。本專案的目標是透過深度學習模型 **EfficientNetB0**，準確區分這兩種寶可夢。
...
### 📊 批量推論結果（Batch Inference Results）
